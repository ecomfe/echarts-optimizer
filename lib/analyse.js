...
function analyse(id, deep, visited, exclude) {
    ...
    if (!moduleInfo) {
        ...
        if (packageInfo && packageInfo.name === moduleId) {
            ...
            var macroVariableDeclaratorName;
            ...
            estraverse.replace(ast, {
                enter: function (node) {
                    ...
                    else if (macro.isMacroVariableDeclarator(node, moduleId)) {
                        // 找到宏模块声明
                        macroVariableDeclaratorName = node.id.name;
                    }
                    else if (macro.isMacroIfStatement(node, macroVariableDeclaratorName)) {
                        console.log(
                            'LOG: Evaluate macro IfStatement [%s, %s, %s]',
                            moduleId,
                            macroVariableDeclaratorName,
                            node.loc.start.line
                        );
                        var newNode = macro.replaceMacroIfStatement(node, macroVariableDeclaratorName);
                        if (newNode === null) {
                            this.remove();
                        }
                        else {
