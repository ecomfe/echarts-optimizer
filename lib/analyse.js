                ...
                switch ( node.type ) {
                    ...
                    case SYNTAX.CallExpression:
                        if ( requireFormalParameter
                            ...
                            var depId = amd.normalize(amd.parseId(requireArg0.value).module, id);
                            addRealDependency(depId);
                        }
                        break;
                }
...
function genBulitCode(moduleInfo) {
    estraverse.replace( moduleInfo.ast, {
        enter: function ( node ) {
            if ( node.type === SYNTAX.CallExpression
                && node.callee.name === 'define'
            ) {
    ...
    moduleInfo.bulitCode = require('escodegen').generate(
        moduleInfo.ast,
    ...
    if (moduleId === packageInfo.name + '/' + packageInfo.main) {
        moduleInfo.bulitCode = 'define(\'' + packageInfo.name 
            + '\', [\'' + moduleId
            + '\'], function (main) {return main;});\n'
            + moduleInfo.bulitCode;
    }
